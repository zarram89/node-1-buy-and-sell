...
В Node.js реализована поддержка четырёх типов потоков:

- `Writable`. Потоки для записи произвольных данных (`createWriteStream()`). Такой поток пригодится, когда требуется записать большой объём данных в файл. Чуть позже рассмотрим на практике;
- `Readable`. Потоки для чтения произвольных данных (`createReadStream()`). Например, чтение большого файла.
- `Duplex`. Потоки для чтения и записи. С таким потоком сталкиваемся во время сетевого взаимодействия между клиентом и сервером.
- `Transform`. Преобразующие потоки. То же самое, что и `Duplex`, но с возможностью модифицировать данные в процессе записи или чтения. Используется реже и необходим во время решения задач с преобразованием данных. Например, сжатие данных перед отправкой.

Функции для работы с потоками доступны в модуле `fs`. Поэтому перед тем, как применять `createWriteStream` или другие перечисленные функции, следует подключить модуль `fs`.
...
Запуск программы инициализирует три потока:

- `stdin` — стандартный поток ввода, поток, принимающий данные для обработки и последующей передачи в поток stdout и/или stderr. Когда в консоли мы запускаем приложение (например, node 01-console-log-and-error.js) мы передаём данные в stdin;
- `stdout` — стандартный поток вывода. Представляет собой поток записи результатов выполнения команд и процессов. Вывод Какой-то текст… попадает в поток stdout;
- `stderr` — стандартный поток ошибок. Как и следует из названия, в этот поток попадают сообщения об ошибках. Следуя нашему примеру кода, в этот поток попадёт строка Какая-то ошибка….

По умолчанию терминал выводит данные для `stdout` и `stderr` вместе. Поэтому мы увидели вывод двух записей, хотя фактически они попали в разные потоки. Потоки позволяют гибко управлять данными приложения. Например, с помощью стандартных возможностей мы можем выполнить запись информации об ошибках в файл, а остальную информацию транслировать в терминал. Рассмотрим практический пример:

```bash
node 01-console-log-and-error.js > app.log 2> error.log
```
После выполнения этой команды рядом со сценарием появятся два файла c логами: app.log и error.log. Первый содержит текст Какой-то текст…, а второй соответственно Какая-то ошибка…. Обратите внимание, сообщения появились в файлах, а в терминале — нет. Мы перенаправили потоки stdout и stderr в файлы.

Для перенаправления потоков мы воспользовались оператором >. А как понять, что мы хотим перенаправлять определённый поток? Для этого мы можем явно указать номер потока. За каждым потоком закреплён отдельный номер:

0 — stdin;
1 — stdout;
2 — stderr.

Для разных языков программирования/платформ существуют готовые инструменты/библиотеки. С их помощью организация процесса логирования упрощается.

Node не является исключением. Для неё создано множество готовых пакетов для логирования на любой вкус и цвет. Наиболее популярные из них: pino, winston, bunyan.

(Экосистема Pino)[https://github.com/pinojs/pino/blob/main/docs/ecosystem.md]

##### Уровни логирования

Как и многие другие популярные пакеты для логирования, Pino поддерживает уровни логирования. Уровни позволяют управлять количеством информации, которое будет регистрировать (логировать) приложение. Например, в продуктовом режиме можно фиксировать события уровня `Info` (информационные), `Warn` (предупреждения) и `Error` (ошибки). В тестовой среде напротив: собирать максимальное количество информации, начиная с уровня `Debug`.

Из коробки доступно несколько предопределённых уровней логирования:

- `fatal` — ошибки, из-за которых приложение не может работать. Например, ошибки, приводящие к полной остановке приложения;
- `error` — ошибки, которые не останавливают работу приложения;
- `warn` — события, которые предупреждают о потенциальной проблеме в приложении;
- `info` — важные штатные действия в приложении;
- `debug` — события, которые могут помочь при отладке приложения;
- `trace` — то же самое, что и debug, только с наименьшим приоритетом. Например, логирование вызовов методов и так далее.
...
```bash
{"level":30,"time":1583075797000,"pid":66257,"hostname":"MacBook-Pro-Igor.local","msg":"Hello, world!","v":1}
{"level":40,"time":1583075797001,"pid":66257,"hostname":"MacBook-Pro-Igor.local","msg":"Test warning","v":1}
{"level":50,"time":1583075797001,"pid":66257,"hostname":"MacBook-Pro-Igor.local","msg":"Add error","v":1}
```
Помимо текста сообщений Pino вывел дополнительную информацию о событии. Обратите внимание, вся информация выводится в формате JSON. При необходимости мы можем запрограммировать отдельный парсер и обработать логи в автоматизированном режиме. Теперь посмотрим из чего состоит объект с описанием события:

- <b>level</b>. Числовое представление уровня логирования (30 — info, 40 — warning, 50 — error);
- <b>time</b>. Время события в формате Unix-time;
- <b>pid</b>. Идентификатор процесса приложения;
- <b>hostname</b>. Название хоста (сервера), на котором запущено приложение;
- <b>msg</b>. Сообщение;
- <b>v</b>. Версия формата журнала записей.

#### Переменные окружения

Переменные окружения (переменные среды) — это некие глобальные значения, расположенные на уровне операционной системы. Приложения могут обращаться к этим переменным и считывать/устанавливать значения, а затем использовать в своих целях.

`printenv` - посмотреть список установленных переменных окружения. `printenv` позволяет прочитать значение произвольной переменной.

```bash
printenv PATH
> /Users/antonov/.nvm/versions/node/v12.13.1/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
```

Есть и альтернативный способ — утилита `echo`. При использовании `echo` вам необходимо указать символ `$` перед именем переменной. Например: `echo $PATH`.

Создать новую переменную окружения можно несколькими способами. От выбранного способа зависит сколько она будет существовать.

- `export`. Утилита позволяет установить переменную для текущего сеанса пользователя. Такая переменная будет доступна до выхода из системы. Пример использования: `export TEST_APP=awesome`.

- `VAR_NAME=value <app>`. Чаще всего переменные нам нужны только во время работы определённого приложения. Например, мы хотим задать порт для поднятия HTTP-сервера через переменные окружения. В этом случае нет смысла создавать переменную для текущего сеанса. Её можно создать во время запуска приложения.

Такая переменная перестанет быть доступной после завершения работы приложения. Рассмотрим пример: `TEST_APP=awesome node my-app.js`. Немного позже мы попрактикуемся на реальных примерах.

Постоянные переменные. Первые два способа справляются с задачей создания временных переменных, но в некоторых случаях может потребоваться создать постоянную переменную. Переменная, которая будет создана сразу после входа в систему. Для этого следует прописать создание переменной в файле `~/.bash_profile` (если вы используете в качестве командного интерпретатора `bash`).
Откройте/создайте в директории пользователя файл `~/.bash_profile` и напишите команду создания переменной: export TEST_APP=awesome. Такая переменная будет создана сразу после входа пользователя в систему. Не забудьте закрыть и снова открыть терминал, прежде чем использовать переменную окружения. Повторное открытие терминала загружает обновлённый файл `~/.bash_profile`.

#### Microsoft Windows

Для взаимодействия с переменными окружения в операционных системах от Microsoft есть свои утилиты. Посмотреть все установленные переменные в Windows поможет встроенная консольная утилита — `se`t. Отройте командный интерпретатор `cmd` и выполните команду `set`.

За чтение содержимого переменной отвечает утилита `ech`o. Имя переменной следует обернуть символами процент (%). Например: `echo %PATH%`.

Мы можем не только читать содержимое установленных переменных, но и создавать новые. Для установки переменной есть несколько способов. От выбранного способа зависит сколько переменная будет существовать.

- `set`. Утилита позволяет создать временную переменную. Например: `set MYAPIKEY=ndsvn2g86nsb9hsg && node app.js`.
- `setx`. Созданная таким способом переменная будет доступна даже после перезагрузки системы. Пример: `setx MYAPIKEY=ndsvn2g86nsb9hsg`.
- Графический интерфейс. В Windows предусмотрено штатное приложение для управления переменными окружениями.
Нажмите сочетание клавиш «Windows + r». Появится окно для ввода команды. Наберите в нём: `sysdm.cpl` и нажмите «OK» (смотри рисунок 4).

#### Установка переменных окружений в одном стиле на разныъ платформах CROSS-ENV
Озвученная выше проблема легко решается с помощью пакета crossenv. Он позволяет выполнять установку переменных окружений в одном стиле на разных платформах. Давайте обновим предыдущий пример. Сначала установим пакет:

```bash
npm i -E cross-env
```

Затем доработаем сценарии запуска:

```js
{
  "scripts": {
    "start::dev": "cross-env NODE_ENV=development node 04-node-env.js",
    "start": "cross-env NODE_ENV=production node 04-node-env.js"
  }
}
```

Теперь сценарии будут одинаково хорошо работать и в Windows, и Linux. Помните, этот вариант установки переменных окружения с помощью сценариев в `package.json` подходит лишь для публичной информации. Этот файл попадает в систему контроля версий, поэтому никаких конфиденциальных данных (токены или что-то ещё) здесь хранить не следует.

#### Много переменных

Теперь сценарии будут одинаково хорошо работать и в Windows, и Linux. Помните, этот вариант установки переменных окружения с помощью сценариев в package.json подходит лишь для публичной информации. Этот файл попадает в систему контроля версий, поэтому никаких конфиденциальных данных (токены или что-то ещё) здесь хранить не следует.

`.env`-файла. В таком файле можно объявить все необходимые переменные, и приложение во время запуска каким—то «магическим» образом их считает и установит. Формат такого файла предельно прост. Каждая новая переменная описывается на отдельной строке. Значение и имя разделяется знаком «=». `.env`-файлы принято размещать в корне проекта.  Пример содержимого файла:
...
Создать .env-файл несложно. Но как его читать? Писать самостоятельно код для его чтения? Вариант рабочий, но эту задачу решили за вас другие разработчики. В npm доступен пакет dotenv. Он берёт на себя задачу чтения .env-файлов. Рассмотрим небольшой практический пример. Полный код примера вы можете найти в репозитории с примерами (смотри 04-hello-env-with-dotenv.js).

```bash
npm i -E dotenv
```

Почитай!!!! 2.10. Потоки (Streams)

#### Декоратор

Декоратор — это самая обычная (почти) функция. При указании имени декоратора мы по факту говорим, какую функцию нужно вызвать.
```js
...
// Снимите комментарий для проверки
